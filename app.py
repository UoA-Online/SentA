...
def pipeline_supports_resumable_runs() -> bool:
    if not hasattr(pipeline, "BatchStoppedError"):
        return False
    try:
        parameters = inspect.signature(pipeline.analyse_rows).parameters
    except (TypeError, ValueError):
        return False
    return "record_callback" in parameters and "row_delay_seconds" in parameters


def stop_if_pipeline_version_mismatch() -> None:
    if pipeline_supports_resumable_runs():
        return
    st.error(
        "Deployment mismatch: `app.py` has the resumable-run UI, but "
        "`testimonial_pipeline.py` is still the older version. Commit and deploy "
        "both files together, then reboot the Streamlit app."
    )
    st.stop()
...
def render_results(result_state: Dict[str, object]) -> None:
    ...
    st.dataframe(visible_output[preview_cols].head(50), width="stretch", hide_index=True)
...
def main() -> None:
    ...
    stop_if_pipeline_version_mismatch()
